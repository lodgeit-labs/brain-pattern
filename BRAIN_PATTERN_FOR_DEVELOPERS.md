...
The convention above describes lesson inheritance between Brains. The commons architecture that lets multiple Brains share a durable interchange surface without any Brain overriding another is documented in the next chapter, *Federation: a contract commons between sovereign brains*.

---

## Federation: a contract commons between sovereign brains

The multi-brain section above describes how Brains can *inherit lessons* from each other under a small explicit convention. That convention says how a receiving Brain writes an inherited candidate into its own canon; it does not say where the inherited candidate *lives* before it is written. Two Brains that never share a write surface can still inherit lessons — by manual paste, by email, by any out-of-band channel — but the channel becomes a mute dependency: nothing enforces that the offering and receiving Brains agree on what was exchanged, when, or under what discipline.

A *contract commons* is a small shared repository sitting between sovereign Brains, holding only the artifacts they explicitly project to each other under a written contract. This chapter describes the commons architecture as it stands after its first full exchange completed between the two production Brains referred to in the multi-brain section above. Everything below is receipts-backed and shape-only — the specific brain names, artifacts, hashes, and amendment identifiers stay on the operators' side.

### 1. Sovereignty

The load-bearing invariant, prior to every other clause: **Brains never write to each other.** No Brain has push access to another Brain's canon; no Brain reads another Brain's private files. The commons repository is the *only* shared write surface between them, and even there each Brain writes only under its own explicit directory. Inheritance from the commons into a Brain's local canon is always a *deliberate act* recorded on the inheriting side — the receiving Brain's own PR, its own operator's ratification, its own local n-count row. Nothing in the commons obliges any Brain to accept anything from it; the commons proposes, sovereign Brains dispose.

This matters because federation without sovereignty degenerates into shared canon under a different name. If Brain A can silently affect Brain B's beliefs by writing to the commons, then Brain A is a co-author of Brain B's canon whether Brain B knows it or not. The one-way discipline — commons proposes, Brain disposes — is what keeps the federation from becoming a distributed monolith.

### 2. The minimal contract

The commons is governed by a short written contract, ratified by every participating Brain's operator, versioned in the commons itself. Each clause names one invariant, and each invariant maps to a specific class of failure the contract prevents. Generalized, the clauses are:

- **Projected artifacts only.** Every artifact in the commons is a *projection* through its source Brain's egress gate, audience-tagged for the commons. Raw canon, secrets, credentials, and operational topology never enter the commons. This is Layer 5 (the Privacy Gradient) applied at the federation boundary: the commons is downhill of every participating Brain's private canon, and content flows only downhill.
- **Mandatory provenance frontmatter.** Every artifact declares source brain, source commit (a SHA in the source Brain's history), content hash (sha256 of the artifact's body), projecting agent, date, and kind (`lesson-offer` / `acceptance` / `contract-amendment` / whatever the contract names). Provenance is not optional and not derivable — the offering Brain writes it at projection time, and the commons carries it as canonical metadata.
- **Citation without replication.** A consuming Brain cites an artifact by its content hash; it does not copy the body into its own canon. Its own ledger row records the hash and its local disposition. This is what makes the commons a shared *reference* surface rather than a shared *canon* — two Brains can cite the same artifact and disagree about what to do with it without either Brain's canon absorbing the other's.
- **Append-only with supersede markers.** Artifacts and ledger rows are never edited or deleted. Corrections, withdrawals, and clarifications are *new* artifacts carrying explicit `supersedes: <content-hash>` metadata. The old artifact stays visible; the new one references it. This is the same supersede-don't-erase discipline the candidate-lesson lifecycle uses, applied to the federation surface.
- **Per-brain sovereign n-counts with composition visible.** n-counts are per-Brain. Each Brain's ledger records its `n_local` and `n_inherited` separately, and cross-brain evidence may count toward a *local* promotion threshold but never toward another Brain's threshold. The commons may carry a reported aggregate tally; it is data, never authority. This closes the shape the multi-brain section above opens — lesson-inheritance-with-composition-visible is a federation invariant, not a stylistic choice.
- **Anchor discipline.** Cross-brain anchors are written `<brain-slug>@<commit>:<path>` (optionally `#<locator>`). An offering Brain cites only anchors it can *itself verify* — references that resolve in its own history at commits it has published. Cross-brain anchors (references to another Brain's canon) are completed by the receiving Brain at acceptance, not fabricated by the offering Brain. Fabricating an unverifiable anchor is a contract violation; declining to cite one you cannot verify is compliance.
- **Tier self-declaration.** Each participating Brain declares in its first artifact which pattern tier it runs (Layer 1 honest-manual, Proofing 0, or Proofing 1). Honesty about tier is part of the contract because it is what makes the evidence a Brain contributes legible to the other Brains.
- **Operator authority.** All credentials are held by operators, one credential per participating agent, scoped to the commons alone. Contract changes are operator-ratified PRs. Ratification is operator-only — no Brain can amend the contract by acting alone, and no Brain can *be* amended without its own operator ratifying.
- **Gates grow with load; absence is declared.** The commons starts at Layer 1 honest-manual (the contract itself plus operator review). Verifiers (frontmatter schema check, content-hash check, append-only assertion, ledger discipline) are added under the six-element rule shape when drift cost warrants — and their absence until then is *declared in the contract*, not hidden. A commons that pretends to enforce what it merely writes down is exactly the class of dishonesty the pattern's tri-state exit contract is designed to prevent.

### 3. Credential topology

A commons that shares credentials shares its sovereignty. The topology that preserves the one-way-write discipline is small and strict:

- **One credential per participating agent.** Each agent that projects into the commons holds its own credential; no credential is shared between agents.
- **Scoped to the commons alone.** Each credential is scoped exactly to the commons repository — not to the source Brain, not to any other repository the operator owns, not to any org-wide surface. A commons credential that also opens the source Brain would collapse the sovereignty guarantee at the auth layer.
- **Isolated store.** The commons credential is stored isolated — in its own file, in its own path, with its own mode-600 permissions — never in the shared credential store the source Brain uses for its own git operations. This is the seed kit's step-6 discipline (operator holds the credential; day-1 credential does not become the federation credential by drift) generalized into federation doctrine.
- **No cross-brain credential possession.** No Brain ever holds another Brain's credential, and no Brain writes to the commons on another Brain's behalf. The commons enforces this at the auth layer: a projection under Brain A's directory succeeds only if authenticated with Brain A's credential, and no operator with access to the commons has access to any other participating Brain's private canon.

The topology is small enough to fit in a paragraph and load-bearing enough that violating any one clause degrades the sovereignty of every clause above it.

### 4. Worked example: the first full exchange

The first full exchange between the two production Brains ran end-to-end within a single day. Told shape-only:

- **Projection.** The offering Brain drafted a lesson-offer artifact, computed the sha256 of the artifact's body under a documented convention, and projected it into its own directory under the commons. The offer carried the mandatory provenance frontmatter: source brain, source commit, content hash, projecting agent, date, kind (`lesson-offer`). Alongside the artifact, a new ledger row referenced the artifact's path and its declared content hash.
- **Verification at read time.** The receiving Brain, at ordering-gate check, discovered the offer. It fetched the artifact bytes via the commons's read surface, recomputed the sha256 of the body under the same convention, and confirmed the byte-identity round-trip. The declared hash matched the recomputed hash: the artifact had crossed the projection boundary intact.
- **Acceptance under the receiving Brain's own gates.** The receiving Brain composed its own acceptance artifact citing the offer *by content hash* (not by re-copying the body — citation without replication in action), declared its own pattern tier, recorded the disposition, and projected the acceptance into its own directory under the commons. A new ledger row referenced the acceptance's own content hash.
- **Sovereignty held at the pressure point.** The offer attributed inherited evidence to the receiving Brain — the shape it named was said to have occurred on the receiving Brain multiple times before. The receiving Brain declined to promote the lesson on that inherited evidence alone. Its own local n-count remained at zero; its acceptance recorded the composition honestly (local n-count, reported inherited n-count) and deferred canon-promotion until either fresh local recurrence or independent re-anchoring by the receiving side. The commons carried the acceptance; the receiving Brain's canon did not carry an unearned promotion. This is the sovereignty invariant working under load: an artifact in the commons cannot promote a lesson into a Brain's canon, no matter how many inherited instances it claims to represent.
- **First genuine design finding.** The exchange also surfaced the contract's first design finding — a real gap the initial contract text had not anticipated. The offering Brain had honestly noted at projection time that it could not verify one class of cross-brain anchor and had refused to fabricate one. The contract as written did not yet name that refusal as compliance rather than defect. Rather than paper over the gap in either the offer or the acceptance, the operators ratified a new amendment: an append-only clarification to the anchor clause, explicitly stating that an offering Brain cites only anchors it can itself verify and that fabricating an unverifiable anchor is a violation — the receiving Brain completes cross-brain anchors at acceptance. The amendment was projected into the commons as its own artifact (`kind: contract-amendment`), appended to the ledger, and superseded no prior clause — it clarified rather than replaced.
- **The amendment is the proof.** The amendment matters as a mechanism, not as a content change: it demonstrates that the contract can *evolve* without violating itself. The append-only invariant held (nothing was edited in place); the operator-authority invariant held (both operators ratified before the amendment landed); the provenance-frontmatter invariant held (the amendment artifact carried the same frontmatter shape as every other artifact); the tier-declaration invariant held (the amendment inherited the offering Brain's tier). A commons whose contract cannot evolve without breaking the contract is a commons that will freeze at first surprise; this commons demonstrated that its evolution mechanism is the same mechanism that governs every artifact it carries. That is the load-bearing property of the exchange.

### 5. Honest limits

What is proven, precisely: the mechanism round-trips a lesson between two sovereign Brains with provenance intact, byte-identity preserved across the projection boundary, sovereignty held at the point where inherited evidence pressed on the receiving Brain's canon, and contract-evolution possible without contract-violation. The first full exchange demonstrated all four properties, days ago at the time of this writing.

What is not yet proven: scale beyond two Brains. Conflict cases where two Brains project mutually-inconsistent artifacts into the commons in the same window. Long-run ledger growth (the current ledger fits in one screen; the maintenance shape of a ledger with thousands of rows is not yet a lived question). Adversarial or careless-operator failure modes. Federation across organisational trust boundaries where the operators do not already have a working relationship. Each of these is a real question the current receipts do not address.

The pattern's credibility here is the honesty of this paragraph. The commons architecture is worth taking seriously because it has done what the previous section claimed multi-brain practice should do; it is not worth taking as a general solution to federated agent-memory because two Brains, one exchange, and a few days of live trial are not a general solution to anything. What the exchange proves is that the shape is viable. Whether the shape survives scale is what the next months of running it will answer.
