...
The first four disciplines above are *maintainer* disciplines — they govern how a curation pass or a schema change is shaped. The two below are *operator* disciplines — they govern how any turn of work advances against the gates the Brain already carries. The distinction matters because they fire at different moments: maintainer disciplines fire when the shape of the Brain is being changed, operator disciplines fire on every turn that closes any unit of work.
...
These six disciplines are not enforced by the Prolog audit — they are how a maintainer keeps the audit's gates *meaningful* over the lifetime of the Brain. The audit catches the failures that survive these disciplines; these disciplines catch the failures the audit cannot see.
