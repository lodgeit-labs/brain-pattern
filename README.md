...
15. **Running the pattern on more than one brain** — the pattern generalises to N Brains under a three-pile factoring (portable core / per-agent adaptation / implementation-specific machinery); tier self-declaration; cross-brain lesson inheritance convention (composition preserved, operator-only promotion, `<brain-slug>@<commit>:<path>` anchor format, offering-brain-cites-only-verifiable-anchors discipline); one anonymized worked example of an inherited candidate lesson recurring locally within 24h and its prevention wire-verified same day.
16. **Federation: a contract commons between sovereign brains** — sovereignty (brains never write to each other; the commons is the ONLY shared write surface; inheritance is always a deliberate act on the inheriting side); minimal contract clause-by-clause (projected-artifacts-only, mandatory provenance frontmatter, citation-without-replication, append-only-with-supersede, per-brain sovereign n-counts with composition visible, anchor discipline, tier self-declaration, operator authority, gates-grow-with-load with absence declared); credential topology (one credential per agent, scoped to the commons alone, isolated store, no cross-brain credential possession); worked example, anonymized (first exchange end-to-end — projection, byte-identity verification, acceptance under receiving brain's gates, sovereignty held at the pressure point, first design finding resolved by operator-ratified append-only amendment); honest limits (two brains, one exchange, days old — what is and is not yet proven).
